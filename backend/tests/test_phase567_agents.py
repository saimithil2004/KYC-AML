...
class TestMonitoringAgent:
    ...
    def test_low_risk_frequency(self):
        ...
        result = run_async(agent.process(state))
        assert (
            result["review_frequency_months"] == MONITORING_FREQUENCY["low"]
        )  # 60 months
    ...
    def test_high_risk_frequency(self):
        ...
        result = run_async(agent.process(state))
        assert (
            result["review_frequency_months"] == MONITORING_FREQUENCY["high"]
        )  # 12 months
...
class TestRegulationAgent:
    ...
    def test_sanctions_block_violation(self):
        ...
        result = run_async(agent.process(state))
        block_violations = [
            v for v in result["violations"] if v.get("rule_type") == "block"
        ]
...
class TestAgentRegistry:
    ...
    def test_all_agents_registered(self):
        ...
        required_agents = [
            ...
            "fatf_agent",
            "ubo_verification_agent",
            "director_verification_agent",
            "account_behavior_agent",
            "regulation_agent",
            "risk_scoring_agent",
            "investigation_agent",
            "decision_agent",
...
class TestOrchestratorAgent:
    ...
    def test_individual_workflow_completes(self):
        ...
        assert final_state.final_decision in (
            "APPROVE",
            "MANUAL_REVIEW",
            "EDD_REQUIRED",
            "REJECT",
    ...
    def test_individual_skips_corporate_agents(self):
        ...
        assert (
            "individual" in logs_joined.lower()
            or "skipping company" in logs_joined.lower()
        )
    ...
    def test_parallel_branches_execute(self):
        ...
        assert len(ran) == len(
            parallel_agents
        ), f"Not all parallel agents ran: missing {set(parallel_agents) - set(ran)}"
