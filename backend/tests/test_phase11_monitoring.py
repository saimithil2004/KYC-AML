...
from app.tasks.schedule_tasks import (
    run_monitoring_screening_task,
    dispatch_periodic_reviews,
    retry_failed_screenings,
)
...
async def test_change_detection_and_job_creation():
    ...
    customer_id = uuid4()
    with patch(
        "app.services.monitoring_service.ensure_phase11_schema", return_value=None
    ):
        job = await MonitoringService.detect_and_trigger_rescreen(
            mock_db, customer_id, "profile_updated"
        )
        ...
        duplicate_job = await MonitoringService.detect_and_trigger_rescreen(
            mock_db, customer_id, "profile_updated"
        )
...
async def test_risk_delta_calculation():
    ...
    mock_res.scalars.return_value.first.side_effect = [
        ...
        ),  # Fourth call fetches monitoring schedule
    ]
    ...
    with patch(
        ...
        "app.services.monitoring_service.ScreeningService.run_screening_async",
        return_value=mock_result,
    ), patch(
        "app.services.monitoring_service.MonitoringService._generate_automated_alerts",
        return_value=None,
    ):

        res = await MonitoringService.run_rescreening(
            mock_db, customer_id, "new_transaction"
        )
        ...
        history_calls = [
            ...
            if isinstance(c[0][0], MonitoringHistory)
        ]
...
def test_celery_dispatch_loop(mock_delay):
    ...
    with patch("app.tasks.schedule_tasks.SessionLocal", return_value=mock_db), patch(
        "app.tasks.schedule_tasks.MonitoringService.detect_and_trigger_rescreen",
