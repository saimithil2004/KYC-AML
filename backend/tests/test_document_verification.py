...
def test_document_validation():
    ...
    # 1. Past Expiry check
    ocr_data = {"expiry_date": {"value": "2020-05-15"}}
    validation = DocumentVerificationService.validate_document(
        doc, ocr_data, is_dup_hash=False
    )
    assert validation["is_expired"] is True

    # 2. Future Expiry check
    ocr_data_valid = {"expiry_date": {"value": "2035-12-31"}}
    validation_valid = DocumentVerificationService.validate_document(
        doc, ocr_data_valid, is_dup_hash=False
    )
