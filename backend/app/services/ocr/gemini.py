...
class GeminiVisionOcr(BaseOcrEngine):
    def __init__(self):
        ...
        self.is_enabled = bool(
            self.api_key and "placeholder" not in self.api_key.lower()
        )
    ...
    def extract_text(
        ...
        if not self.is_enabled:
            logger.info(
                "Gemini API key not configured; using high-fidelity simulated Gemini OCR results."
            )
    ...
    def _simulate_gemini_extraction(
        self, document_type: str, file_name: str
    ) -> Dict[str, Dict[str, Any]]:
        name_hint = "John Doe"
        ...
        base = {
            ...
            "address": {
                "value": "123 Baker Street, London, W1A 1AA, United Kingdom",
                "confidence": 0.95,
            },
