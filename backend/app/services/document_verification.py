...
try:
    ...
    def calculate_similarity(s1: str, s2: str) -> float:
        if not s1 or not s2:
            return 0.0
        return round(SequenceMatcher(None, s1.strip().lower(), s2.strip().lower()).ratio() * 100, 1)
...
class DocumentVerificationService:
    ...
    def assess_image_quality(file_path: Path) -> Dict[str, Any]:
        ...
        if size < 5 * 1024:  # Under 5KB is likely unreadable or corrupt
            ...
            metrics["reasons"].append("File size is too small; resolution check failed.")
    ...
    @classmethod
    def validate_document(cls, doc: Document, ocr_data: Dict[str, Any], is_dup_hash: bool) -> Dict[str, Any]:
        ...
        if doc.content_type and doc.content_type not in allowed_mimes:
            ...
            validation["errors"].append(f"MIME type '{doc.content_type}' is not supported.")

        # Expiry Validation
        expiry_field = ocr_data.get("expiry_date", {})
        expiry_val = expiry_field.get("value") if isinstance(expiry_field, dict) else expiry_field
        if expiry_val:
            try:
                exp_date = (
    ...
    @classmethod
    def match_kyc_data(cls, ocr_data: Dict[str, Any], kyc: KYCProfile, customer: Customer) -> Dict[str, Any]:
        ...
        ocr_name = ocr_name_field.get("value") if isinstance(ocr_name_field, dict) else ocr_name_field
        declared_name = kyc.full_name if kyc else f"{customer.first_name or ''} {customer.last_name or ''}".strip()
        matching["name_match_ratio"] = calculate_similarity(ocr_name or "", declared_name)

        # DOB match
        ocr_dob_field = ocr_data.get("dob", {})
        ocr_dob = ocr_dob_field.get("value") if isinstance(ocr_dob_field, dict) else ocr_dob_field
        ...
        ocr_addr = ocr_addr_field.get("value") if isinstance(ocr_addr_field, dict) else ocr_addr_field
        declared_addr = kyc.address if kyc else f"{customer.street_address or ''}, {customer.city or ''}".strip()
        matching["address_match_ratio"] = calculate_similarity(ocr_addr or "", declared_addr)

        # Nationality match
        ocr_nat_field = ocr_data.get("nationality", {})
        ocr_nat = ocr_nat_field.get("value") if isinstance(ocr_nat_field, dict) else ocr_nat_field
    ...
    @staticmethod
    def scan_for_fraud(file_path: Path, ocr_data: Dict[str, Any], db_session, current_doc_id: UUID) -> List[str]:
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                edit_signatures = [b"Adobe Photoshop", b"Photoshop", b"GIMP", b"Pixelmator"]
                for sig in edit_signatures:
                    if sig in content:
                        fraud_reasons.append(f"Image manipulation warning: metadata contains '{sig.decode()}' signature.")
        except Exception:
            pass

        # Document Number reuse check
        doc_num_field = ocr_data.get("document_number", {})
        doc_num = doc_num_field.get("value") if isinstance(doc_num_field, dict) else doc_num_field
        if doc_num:
            other_doc = db_session.query(Document).filter(
                Document.id != current_doc_id,
                Document.ocr_data["document_number"]["value"].astext == doc_num
            ).first()
            if other_doc:
                fraud_reasons.append(f"Duplicate document number reuse detected: matched customer {other_doc.customer_id}")
    ...
    def run_pre_ocr_checks(cls, db_session, document_id: UUID) -> Dict[str, Any]:
        ...
        if not quality["passed"]:
            ...
            doc.verification_metadata["validation"] = {
                "errors": ["Image quality checks failed: " + ", ".join(quality["reasons"])]
            }
            db_session.commit()
    ...
    @classmethod
    def run_post_ocr_verification(cls, db_session, document_id: UUID, confirmed_ocr_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        file_path = Path(doc.file_path)
        kyc = db_session.query(KYCProfile).filter(KYCProfile.customer_id == doc.customer_id).first()
        customer = db_session.query(Customer).filter(Customer.id == doc.customer_id).first()
        ...
        if file_hash:
            match = db_session.query(Document).filter(
                Document.id != doc.id,
        ...
        # Fraud Agent
        fraud_reasons = cls.scan_for_fraud(file_path, confirmed_ocr_data, db_session, doc.id)
        ...
        if matching["overall_confidence_score"] < 60.0 and kyc:
            risk_score += 30.0
            reasons.append(f"Low match confidence ratio: {matching['overall_confidence_score']}%")
