...
class AuthService:
    ...
    async def revoke_token(
        db: AsyncSession, jti: str, user_id: Optional[UUID], reason: str = "logout"
    ) -> None:
    ...
    ) -> Optional[MFASettings]:
        result = await db.execute(
            select(MFASettings).where(MFASettings.user_id == user_id)
        )
    ...
    async def confirm_mfa(db: AsyncSession, user: User, code: str) -> bool:
        ...
        if verify_totp(secret, code):
            ...
            await AuditService.log(
                ...
                user_id=user.id,
                action="MFA_SETUP",
                entity_name="mfa_settings",
                entity_id=mfa_set.id,
            )
            return True
        return False
