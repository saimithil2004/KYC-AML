...
class ReportService:
    ...
    async def compile_report_data(
        db: AsyncSession, report_type: str, filters: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        # ─── 2. High Risk Customers ───
            ...
            q = (
                ...
                .where(
                    or_(
                        KYCProfile.risk_category == "high",
                        KYCProfile.risk_category == "critical",
                    )
                )
            )
            res = await db.execute(q)
                ...
                data.append(
                    {
                        ...
                        "score": (
                            float(kyc.risk_score) if kyc and kyc.risk_score else 0.0
                        ),
                        "nationality": kyc.nationality if kyc else "N/A",
                    }
                )
        ...
        # ─── 3. Risk Distribution ───
            ...
            q = select(
                KYCProfile.risk_category, func.count(KYCProfile.id).label("count")
            ).group_by(KYCProfile.risk_category)
            res = await db.execute(q)
            rows = res.all()
            for r in rows:
                data.append({"risk_category": r[0] or "unknown", "count": r[1]})
        ...
        # ─── 6. Case Statistics ───
            ...
            q = select(
                Case.status, Case.priority, func.count(Case.id).label("count")
            ).group_by(Case.status, Case.priority)
            res = await db.execute(q)
            rows = res.all()
            for r in rows:
                data.append({"status": r[0], "priority": r[1], "count": r[2]})

        # ─── 7. Alert Statistics ───
        elif report_type == "alert_statistics":
            q = select(
                Alert.alert_type, Alert.status, func.count(Alert.id).label("count")
            ).group_by(Alert.alert_type, Alert.status)
            res = await db.execute(q)
            rows = res.all()
            for r in rows:
                data.append({"alert_type": r[0], "status": r[1], "count": r[2]})

        # ─── 8. SAR Statistics ───
        elif report_type == "sar_statistics":
            q = select(SAR.status, func.count(SAR.id).label("count")).group_by(
                SAR.status
            )
            res = await db.execute(q)
            rows = res.all()
            for r in rows:
                data.append({"status": r[0], "count": r[1]})
        ...
        # ─── 9. Investigation Statistics ───
            ...
            rows = res.all()
            for r in rows:
                data.append({"status": r[0], "risk_level": r[1], "count": r[2]})
        ...
        # ─── 13. Agent Performance ───
            ...
            rows = res.all()
            for r in rows:
                data.append({"agent_name": r[0], "step_name": r[1], "count": r[2]})

        # ─── 14. Dashboard KPIs ───
        elif report_type == "dashboard_kpis":
            c_count = (
                await db.execute(select(func.count(Customer.id)))
            ).scalar_one() or 0
            a_count = (await db.execute(select(func.count(Alert.id)))).scalar_one() or 0
            case_count = (
                await db.execute(select(func.count(Case.id)))
            ).scalar_one() or 0
            tx_sum = (
                await db.execute(select(func.sum(Transaction.amount)))
            ).scalar_one() or 0.0
            data.append(
                {
    ...
    def generate_pdf_bytes(
        data: List[Dict[str, Any]], title: str, filters: Dict[str, Any]
    ) -> bytes:
        ...
        if data:
            headers = list(data[0].keys())
            table_headers = [
                Paragraph(f"<b>{h.replace('_', ' ').title()}</b>", body_style)
                for h in headers
            ]
            ...
            for idx, item in enumerate(
                data[:45]
            ):  # cap to fit on standard 1-2 pages gracefully
                row_cells = []
            ...
            story.append(
                Paragraph(
                    "<i>No data records found matching report criteria.</i>", body_style
                )
            )
        ...
        story.append(
            Paragraph(
                "CONFIDENTIAL · AML/KYC Platform BI Generated Document Output Footer",
                bold_style,
            )
        )
