...
from app.models.models import (
    Account, Alert, Case, Customer, KYCProfile, Transaction,
)
...
FATF_HIGH_RISK_COUNTRIES = {
    "Afghanistan", "Myanmar", "North Korea", "Iran", "Syria",
    "Yemen", "Libya", "Somalia", "South Sudan", "Venezuela",
    "Haiti", "Panama", "Philippines", "Pakistan", "Uganda",
    "Nigeria", "Burkina Faso", "Mali", "Mozambique",
    "Cameroon", "Senegal", "South Africa", "Tanzania", "Vietnam",
}
...
async def analyse_and_alert(
    ...
    if existing_case:
        ...
        max_score = max(ALERT_LEVEL_MAP.get(p[0], ("MEDIUM", 60.0))[1] for p in patterns)
...
async def _detect_patterns(
    ...
    circ_txs = [
        t for t in all_txs
        if t.created_at >= circ_start
        and t.receiver_account_number == account_num
    ]
    if circ_txs:
        patterns.append((
            "CIRCULAR_TRANSFER",
    ...
    if hist_txs:
        ...
        today_total = sum(float(t.amount) for t in all_txs if t.created_at >= today_start)
    ...
    if len(all_txs[:50]) >= 5 and len(cash_txs) / max(len(all_txs[:50]), 1) >= CASH_INTENSIVE_PCT:
        patterns.append((
            "CASH_INTENSIVE",
    ...
    inbound = [t for t in all_txs if float(t.amount) < 500 and t.transaction_type in ("credit", "inbound")]
    outbound = [t for t in all_txs if float(t.amount) > 2000 and t.transaction_type in ("debit", "outbound", "transfer")]
...
def _trigger_rescreening_background(customer_id: str) -> None:
    ...
    def _run():
        try:
            ...
            logger.info(f"TransactionMonitoring: Background re-screening complete for {customer_id}")
        except Exception as exc:
            logger.error(f"TransactionMonitoring: Background re-screening failed for {customer_id}: {exc}")
