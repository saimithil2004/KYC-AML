    ...
    def _extract_deterministically(text: str) -> List[Dict[str, Any]]:
        ...
        for idx, sentence in enumerate(sentences):
            ...
            match_amount = re.search(
                r"(?:exceeding|over|greater than|above)\s*([£$€]?\s*\d{1,3}(?:,\d{3})*(?:\.\d{2})?)",
                lowered,
            )
            if match_amount:
                amount_str = (
                ...
                try:
                    ...
                    rule_type = (
                        "edd"
                        if "edd" in lowered or "enhanced" in lowered
                        else "threshold"
                    )
                    rules.append(
                        {
            ...
            if (
                "high risk country" in lowered
                or "jurisdiction" in lowered
                or "sanction" in lowered
            ):
                rule_type = (
                    "block" if "block" in lowered or "prohibit" in lowered else "edd"
                )
                rules.append(
                    {
