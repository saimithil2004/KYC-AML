    ...
    if customer_in.customer_type == "corporate":
        ...
        result = await db.execute(
            select(Company).where(
                Company.registration_number == customer_in.company.registration_number
            )
        )
...
async def get_my_company(
    ...
    dir_result = await db.execute(
        select(Director).where(Director.company_id == company.id)
    )
        ...
        "ubos": [
            {
                ...
                "ownership_percentage": (
                    float(ubo_in.ownership_percentage)
                    if hasattr(ubo_in := u, "ownership_percentage")
                    else 0
                ),
...
async def get_customer_by_id(
    ...
    if customer.user_id != current_user.id and current_user.role not in [
        ...
        raise HTTPException(
            status_code=403, detail="Not authorized to access this customer profile."
        )
...
async def delete_customer(
    ...
    if customer.user_id != current_user.id and current_user.role != "admin":
        raise HTTPException(
            status_code=403,
            detail="Only the profile owner or administrator can delete this profile.",
        )
