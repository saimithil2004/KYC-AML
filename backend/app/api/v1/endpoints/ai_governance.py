...
from app.models.models import (
    User, AIModel, ModelVersion, PromptTemplate, PromptVersion,
...
verify_auditor_or_above = RoleChecker(["auditor", "compliance_officer", "admin", "ai_admin"])
...
@router.get("/models", response_model=List[Dict[str, Any]], dependencies=[Depends(verify_auditor_or_above)])
async def list_models(db: AsyncSession = Depends(get_db)):
    ...
    for m in models:
        ...
        out.append({
            ...
            "versions": [{"id": str(v.id), "version": v.version, "is_active": v.is_active, "metadata": v.metadata_json} for v in versions]
        })
    return out

@router.post("/models", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_ai_admin)])
async def create_model(model_in: AIModelCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Register a new model (AI Admin only)."""
    model = await AIGovernanceService.register_model(db, model_in.name, model_in.provider, model_in.is_active)
    ...
    return {"id": str(model.id), "name": model.name, "provider": model.provider, "is_active": model.is_active}

@router.post("/models/{model_id}/versions", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_ai_admin)])
async def create_model_version(model_id: UUID, version_in: ModelVersionCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Register a version snapshot under a model (AI Admin only)."""
    mv = await AIGovernanceService.register_model_version(db, model_id, version_in.version, version_in.metadata_json)
    await AuditService.log(
        db=db,
    ...
    return {"id": str(mv.id), "version": mv.version, "is_active": mv.is_active, "metadata": mv.metadata_json}

@router.put("/models/{model_id}", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def update_model(model_id: UUID, model_in: AIModelUpdate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.delete("/models/{model_id}", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def delete_model(model_id: UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.get("/prompts", response_model=List[Dict[str, Any]], dependencies=[Depends(verify_auditor_or_above)])
async def list_prompts(db: AsyncSession = Depends(get_db)):
    ...
    out = []
    for t in templates:
        v_stmt = select(PromptVersion).where(PromptVersion.template_id == t.id).order_by(desc(PromptVersion.created_at))
        v_res = await db.execute(v_stmt)
        versions = v_res.scalars().all()
...
@router.post("/prompts", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_ai_admin)])
async def create_prompt_template(temp_in: PromptTemplateCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Register prompt template mapping (AI Admin only)."""
    t = await AIGovernanceService.create_prompt_template(db, temp_in.name, temp_in.description)
    await AuditService.log(
        db=db,
...
@router.post("/prompts/{template_id}/versions", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_ai_admin)])
async def create_prompt_version(template_id: UUID, version_in: PromptVersionCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Create a draft version revision under template (AI Admin only)."""
    pv = await AIGovernanceService.create_prompt_version(db, template_id, version_in.content, version_in.version, version_in.status)
    await AuditService.log(
        db=db,
...
@router.put("/prompts/{template_id}", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def update_prompt_template(template_id: UUID, temp_in: PromptTemplateCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.delete("/prompts/{template_id}", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def delete_prompt_template(template_id: UUID, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.get("/executions", response_model=Dict[str, Any], dependencies=[Depends(verify_compliance_or_admin)])
...
@router.get("/executions/{execution_id}", response_model=Dict[str, Any], dependencies=[Depends(verify_compliance_or_admin)])
...
@router.get("/explanations/{execution_id}", response_model=Dict[str, Any], dependencies=[Depends(verify_compliance_or_admin)])
async def get_explanation(execution_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not exp:
        raise HTTPException(status_code=404, detail="Explainability report not found for this execution.")
...
@router.post("/feedback", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_compliance_or_admin)])
async def submit_human_feedback(fb_in: FeedbackCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.post("/approve", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def approve_prompt(app_in: ApprovalSubmit, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Reviewer approves prompt template version revision (AI Admin only)."""
    pv = await AIGovernanceService.approve_prompt_version(db, app_in.version_id, current_user.id, app_in.comments)
    await AuditService.log(
        db=db,
...
@router.post("/reject", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def reject_prompt(app_in: ApprovalSubmit, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Reject prompt template version revision (AI Admin only)."""
    pv = await AIGovernanceService.reject_prompt_version(db, app_in.version_id, current_user.id, app_in.comments)
    await AuditService.log(
        db=db,
...
@router.post("/rollback", response_model=Dict[str, Any], dependencies=[Depends(verify_ai_admin)])
async def rollback_prompt(roll_in: RollbackSubmit, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.get("/policies", response_model=List[Dict[str, Any]], dependencies=[Depends(verify_auditor_or_above)])
...
@router.post("/policies", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_ai_admin)])
async def create_policy(policy_in: AIPolicyCreate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
@router.get("/statistics", response_model=Dict[str, Any], dependencies=[Depends(verify_auditor_or_above)])
...
@router.get("/dashboard", response_model=Dict[str, Any], dependencies=[Depends(verify_auditor_or_above)])
...
@router.get("/provider-status", response_model=Dict[str, Any], dependencies=[Depends(verify_auditor_or_above)])
...
@router.get("/evaluations", response_model=List[Dict[str, Any]], dependencies=[Depends(verify_auditor_or_above)])
...
@router.post("/evaluate", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_compliance_or_admin)])
async def submit_evaluation(eval_in: ModelEvaluationSubmit, db: AsyncSession = Depends(get_db)):
