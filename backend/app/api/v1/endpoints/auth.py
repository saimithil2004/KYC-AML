...
async def login(
    ...
    if mfa_set and mfa_set.enabled:
        if not user_in.mfa_code:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="MFA_REQUIRED"
            )
...
async def refresh_token(
    ...
    if jti:
        await AuthService.revoke_token(
            db, jti, user.id, reason="refresh_token_rotation"
        )
...
async def logout(
    ...
    if refresh_tok:
        try:
            ...
            if jti:
                await AuthService.revoke_token(
                    db, jti, UUID(user_id) if user_id else None, reason="logout"
                )
        except Exception:
            pass
