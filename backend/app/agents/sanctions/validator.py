...
class SanctionsValidator:
    ...
    def build_individual_subjects(state: AgentState) -> List[ScreeningSubject]:
        ...
        for subj in subjects:
            ...
            if subj.role == ROLE_CUSTOMER:
                passport = (
                    state.customer.get("passport_number") or
                    state.customer_profile.get("passport_number") or
                    (state.kyc_profile.get("passport_number") if state.kyc_profile else None)
                )
            elif subj.role == "DIRECTOR":
                for d in state.directors or []:
                    if d.get("name") == subj.full_name or f"{d.get('first_name', '')} {d.get('last_name', '')}".strip() == subj.full_name:
                        passport = d.get("passport_number")
                        break
            elif subj.role == "UBO":
                for u in state.ubos or []:
                    if u.get("name") == subj.full_name or f"{u.get('first_name', '')} {u.get('last_name', '')}".strip() == subj.full_name:
                ...
                for s in shareholders:
                    if s.get("name") == subj.full_name or f"{s.get('first_name', '')} {s.get('last_name', '')}".strip() == subj.full_name:
    ...
    def build_company_subjects(state: AgentState) -> List[CompanyScreeningSubject]:
        ...
        for idx, comp in enumerate(state.companies or []):
            ...
            if key not in seen_keys:
                seen_keys.add(key)
                companies.append(CompanyScreeningSubject(
                    subject_id=f"subj-comp-{idx}-{uuid.uuid4().hex[:6]}",
                    role=ROLE_COMPANY,
                    company_name=name_str,
                    registration_number=reg_str or None,
                    country=comp.get("country") or comp.get("registered_country") or None,
                    business_address=(
                        comp.get("registered_address") or
                        comp.get("business_address") or
                        comp.get("address") or None
                    )
                ))
        ...
        if customer_type == "business" and not state.companies:
            name = state.customer.get("company_name") or state.customer.get("name") or f"{state.customer.get('first_name', '')} {state.customer.get('last_name', '')}".strip()
            if name:
                name_str = str(name).strip()
                reg_num = state.customer.get("registration_number") or state.customer_profile.get("registration_number")
                reg_str = str(reg_num).strip() if reg_num else ""
                key = (name_str.lower(), reg_str.lower())
                if key not in seen_keys:
                    seen_keys.add(key)
                    companies.append(CompanyScreeningSubject(
                        ...
                        company_name=name_str,
                        registration_number=reg_str or None,
                        country=state.customer.get("country") or state.customer.get("registered_country") or None,
                        business_address=state.customer.get("business_address") or state.customer.get("address") or None
                    ))
    ...
    def validate_subjects(
        individuals: List[ScreeningSubject],
        companies: List[CompanyScreeningSubject]
    ) -> List[str]:
        """
        ...
        for ind in individuals:
            if not ind.dob:
                warnings.append(f"Individual '{ind.full_name}' is missing Date of Birth. Match accuracy reduced.")
            if not ind.passport_number:
                warnings.append(f"Individual '{ind.full_name}' is missing Passport Number. Passport matching disabled.")

        for comp in companies:
            if not comp.registration_number:
                warnings.append(f"Company '{comp.company_name}' is missing Registration Number. Company reg match disabled.")
            if not comp.country:
                warnings.append(f"Company '{comp.company_name}' is missing Country. Country matching disabled.")
