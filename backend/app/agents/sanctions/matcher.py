...
from app.agents.screening.constants import (
    ...
    ENTITY_INDIVIDUAL, ENTITY_COMPANY
)
...
class SanctionsMatcher:
    ...
    def score_individual(
        ...
        candidates: List[SanctionRecord]
    ) -> SanctionMatchResult:
        """
        ...
        for record in candidates:
            score, reason, matched_fields = SanctionsMatcher._compute_individual_score(subject, record)
            if score > best_score:
                best_score = score
                best_record = record
                best_reason = reason
    ...
    def score_company(
        ...
        for record in candidates:
            score, reason, matched_fields = SanctionsMatcher._compute_company_score(subject, record)
            if score > best_score:
                best_score = score
                best_record = record
                best_reason = reason
    ...
    def _compute_individual_score(
        ...
        """
        score, reasons, dob_match, nat_match, name_sim = BaseScreeningMatcher.compute_individual_score(
            subject_name=subject.full_name,
        ...
        if subject.country and record.country and subject.country.strip().lower() == record.country.strip().lower():
        ...
        if passport_matched:
            ...
            if name_sim >= 70:
                score = 100.0
                reasons.append("Passport number exact match (Name similarity >= 70% -> CONFIRMED)")
            else:
                # Name is too different, cap at POSSIBLE_MATCH to prevent identity theft false positives
                score = min(score, THRESHOLD_POSSIBLE)
                reasons.append("Passport number exact match but Name similarity too low (< 70% -> Capped)")
        else:
        ...
        if name_sim >= 80.0:
            ...
            reasons.append(f"(high name similarity {name_sim:.0f}% → POSSIBLE_MATCH override)")
    ...
    def _compute_company_score(
        ...
        if reg_matched:
            ...
            if name_sim >= 70:
                score = 100.0
                reasons.append("Exact Registration Number Match (Name similarity >= 70% -> CONFIRMED)")
            else:
                score = min(score, THRESHOLD_POSSIBLE)
                reasons.append("Registration Number Match but Company Name similarity too low (< 70% -> Capped)")
        else:
            # Exact name + country match -> CONFIRMED override
            if name_sim == 100.0 and country_matched:
                score = max(score, THRESHOLD_CONFIRMED)
                reasons.append("(exact company name + country match → CONFIRMED override)")
            # High company name similarity -> at least POSSIBLE
            elif name_sim >= 80.0:
                score = max(score, THRESHOLD_POSSIBLE)
                reasons.append(f"(high company name similarity {name_sim:.0f}% → POSSIBLE_MATCH override)")
            else:
    ...
    def _no_match_result(
        subject: Union[ScreeningSubject, CompanyScreeningSubject],
        entity_type: str
    ) -> SanctionMatchResult:
        name = subject.full_name if isinstance(subject, ScreeningSubject) else subject.company_name
        return SanctionMatchResult(
            subject_id=subject.subject_id,
            subject_name=name,
            subject_role=subject.role,
