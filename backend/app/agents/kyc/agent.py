...
class KycAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> Dict[str, Any]:
        ...
        score_personal, missing_personal, passed_personal = KycValidator.validate_personal_info(customer)
        score_address, missing_address, passed_address = KycValidator.validate_address(customer)
        score_docs, missing_docs, passed_docs = KycValidator.validate_identity_documents(docs)
        score_occupation, missing_occupation, passed_occupation = KycValidator.validate_occupation(kyc_profile)
        score_sof, missing_sof, passed_sof = KycValidator.validate_source_of_funds(kyc_profile)
        score_sow, missing_sow, passed_sow = KycValidator.validate_source_of_wealth(kyc_profile)
        score_tax, missing_tax, passed_tax = KycValidator.validate_tax_residency(kyc_profile)
        ...
        eval_result = KycRulesEngine.evaluate(customer, kyc_profile, docs, missing_fields)
        ...
        state.logs.append(f"KycAgent screening finished. Score: {kyc_score}, Status: {kyc_status}, Routing to: {next_agent}")
