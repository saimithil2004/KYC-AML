...
class User(Base):
    ...
    password_changed_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=True)
    ...
    # Relationships
    mfa_settings: Mapped[Optional["MFASettings"]] = relationship(back_populates="user", uselist=False)
...
class Customer(Base):
    ...
    monitoring_jobs: Mapped[List["MonitoringJob"]] = relationship(back_populates="customer", cascade="all, delete-orphan")
    monitoring_histories: Mapped[List["MonitoringHistory"]] = relationship(back_populates="customer", cascade="all, delete-orphan")
    investigations: Mapped[List["Investigation"]] = relationship(back_populates="customer", cascade="all, delete-orphan")
...
class Case(Base):
    ...
    investigation: Mapped[Optional["Investigation"]] = relationship(back_populates="case", uselist=False)
...
class Regulation(Base):
    ...
    status: Mapped[str] = mapped_column(String(50), default="active")  # active, archived, draft, pending_review
...
class PolicyRule(Base):
    ...
    rule_type: Mapped[str] = mapped_column(String(100), nullable=False)  # 'threshold', 'block', 'edd', 'aml', 'kyc', 'internal'
...
class RegulationVersion(Base):
    ...
    author_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class MonitoringJob(Base):
    ...
    case_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("cases.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class MonitoringHistory(Base):
    ...
    risk_trend: Mapped[str] = mapped_column(String(50), default="stable")  # improving, stable, deteriorating
    agents_executed: Mapped[dict] = mapped_column(JSONB, nullable=False)  # stores list of agent names as json array
    execution_time_ms: Mapped[int] = mapped_column(Integer, default=0)
    case_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("cases.id", ondelete="SET NULL"), nullable=True)
    risk_score_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("risk_scores.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class Investigation(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    case_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("cases.id", ondelete="CASCADE"), nullable=False, unique=True)
    customer_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("customers.id", ondelete="CASCADE"), nullable=False)
    assigned_to: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    assigned_supervisor_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="open")  # open, under_review, escalated, edd_required, closed
    risk_level: Mapped[str] = mapped_column(String(50), default="medium")
    ai_summary: Mapped[Optional[dict]] = mapped_column(JSONB)  # case_summary, suspicious_behaviour_analysis, recommended_next_actions, questions_for_investigator, missing_evidence_suggestions, risk_explanation
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    assigned_officer: Mapped[Optional["User"]] = relationship(foreign_keys=[assigned_to])
    supervisor: Mapped[Optional["User"]] = relationship(foreign_keys=[assigned_supervisor_id])
    evidences: Mapped[List["Evidence"]] = relationship(back_populates="investigation", cascade="all, delete-orphan")
    notes: Mapped[List["CaseNote"]] = relationship(back_populates="investigation", cascade="all, delete-orphan")
    sars: Mapped[List["SAR"]] = relationship(back_populates="investigation", cascade="all, delete-orphan")
    timeline_events: Mapped[List["TimelineEvent"]] = relationship(back_populates="investigation", cascade="all, delete-orphan")
    assignments: Mapped[List["Assignment"]] = relationship(back_populates="investigation", cascade="all, delete-orphan")
...
class Evidence(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    investigation_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("investigations.id", ondelete="CASCADE"), nullable=False)
    file_name: Mapped[str] = mapped_column(String(255), nullable=False)
    evidence_type: Mapped[str] = mapped_column(String(50), nullable=False)  # pdf, docx, image, csv, zip, audio, video
    file_path: Mapped[str] = mapped_column(Text, nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text)
    uploaded_by: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class SAR(Base):
    ...
    risk_indicators: Mapped[dict] = mapped_column(JSONB, nullable=False)  # JSON list of risk indicator tags
    recommendation: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(String(50), default="draft")  # draft, submitted, approved, rejected, archived
    created_by: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Assignment(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    investigation_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("investigations.id", ondelete="CASCADE"), nullable=False)
    assigned_by: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    assigned_to: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    role: Mapped[str] = mapped_column(String(50), default="investigator")  # investigator, supervisor
...
class ReportTemplate(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    config: Mapped[dict] = mapped_column(JSONB, nullable=False)  # fields, charts, tables config
    created_by: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Report(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    template_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("report_templates.id", ondelete="SET NULL"), nullable=True)
    generated_by: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="pending")  # pending, completed, failed
    format: Mapped[str] = mapped_column(String(20), default="pdf")  # pdf, excel, csv, json
    file_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    filters: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)  # date range, customer, countries, etc.
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class ScheduledReport(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    template_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("report_templates.id", ondelete="CASCADE"), nullable=False)
    cron_expression: Mapped[str] = mapped_column(String(100), nullable=False)  # daily, weekly, etc. or standard cron
    next_run: Mapped[datetime] = mapped_column(DateTime, nullable=False)
    status: Mapped[str] = mapped_column(String(50), default="active")  # active, paused, deleted
    created_by: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class ReportExecution(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    schedule_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("scheduled_reports.id", ondelete="CASCADE"), nullable=False)
    report_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("reports.id", ondelete="SET NULL"), nullable=True)
...
class DashboardLayout(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    user_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=True)
    ...
    widgets: Mapped[List["DashboardWidget"]] = relationship(back_populates="layout", cascade="all, delete-orphan")
...
class DashboardWidget(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    layout_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("dashboard_layouts.id", ondelete="CASCADE"), nullable=False)
    widget_type: Mapped[str] = mapped_column(String(100), nullable=False)  # chart_bar, chart_pie, kpi_card, etc.
    title: Mapped[str] = mapped_column(String(255), nullable=False)
...
class IntegrationSetting(Base):
    ...
    provider_type: Mapped[str] = mapped_column(String(50), nullable=False)  # sanctions, pep, company, notification
...
class SyncHistory(Base):
    ...
    sync_type: Mapped[str] = mapped_column(String(50), nullable=False)  # manual, scheduled, incremental
...
class NotificationTemplate(Base):
    ...
    channel: Mapped[str] = mapped_column(String(50), nullable=False)  # email, slack, teams, in_app
...
class Notification(Base):
    ...
    template_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("notification_templates.id", ondelete="SET NULL"), nullable=True)
    channel: Mapped[str] = mapped_column(String(50), nullable=False)  # email, slack, teams, in_app, webhook
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    message: Mapped[str] = mapped_column(Text, nullable=False)
    priority: Mapped[str] = mapped_column(String(20), default="medium")  # low, medium, high, urgent
    status: Mapped[str] = mapped_column(String(50), default="pending")   # pending, sent, failed, retry
...
class WebhookLog(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    endpoint_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("webhook_endpoints.id", ondelete="CASCADE"), nullable=False)
    ...
    status: Mapped[str] = mapped_column(String(50), default="pending")  # success, failed, retry
    retry_count: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
...
class LoginHistory(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    email: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, index=True)  # Store email even if user not found
    ip_address: Mapped[Optional[str]] = mapped_column(String(45), nullable=True)          # IPv4 or IPv6
    user_agent: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    device_fingerprint: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    success: Mapped[bool] = mapped_column(Boolean, default=False)
    failure_reason: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)      # wrong_password, account_locked, etc.
    country: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
...
class RevokedToken(Base):
    ...
    user_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=True)
    token_type: Mapped[str] = mapped_column(String(20), default="access")  # access / refresh
    reason: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)  # logout, password_changed, admin_revoke
    revoked_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, index=True)
    expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)  # Auto-cleanup after expiry
...
class MFASettings(Base):
    ...
    secret: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)        # Encrypted TOTP secret
    backup_codes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)         # JSON list of hashed backup codes
...
class SecurityEvent(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    event_type: Mapped[str] = mapped_column(String(100), nullable=False, index=True)  # ACCOUNT_LOCKED, BRUTE_FORCE, etc.
...
class SystemMetric(Base):
    ...
    unit: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)   # %, ms, MB, req/s, etc.
...
class BackupRecord(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    backup_type: Mapped[str] = mapped_column(String(50), nullable=False)   # database, documents, config, full
    file_path: Mapped[str] = mapped_column(String(1000), nullable=False)
    file_name: Mapped[str] = mapped_column(String(500), nullable=False)
    file_size_bytes: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    sha256_checksum: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="pending")      # pending, completed, failed, verified
    triggered_by: Mapped[str] = mapped_column(String(50), default="scheduled")  # scheduled, manual, admin
    error_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class ModelVersion(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    model_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ai_models.id", ondelete="CASCADE"), nullable=False)
    version: Mapped[str] = mapped_column(String(50), nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    ...
    executions: Mapped[List["AIExecution"]] = relationship(back_populates="model_version")
    evaluations: Mapped[List["ModelEvaluation"]] = relationship(back_populates="model_version", cascade="all, delete-orphan")
...
class PromptVersion(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    template_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("prompt_templates.id", ondelete="CASCADE"), nullable=False)
    version: Mapped[str] = mapped_column(String(50), nullable=False)
    content: Mapped[str] = mapped_column(Text, nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=False)
    approved_status: Mapped[str] = mapped_column(String(50), default="pending")  # 'draft', 'pending', 'approved', 'rejected', 'deprecated'
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    template: Mapped["PromptTemplate"] = relationship(back_populates="versions")
    executions: Mapped[List["AIExecution"]] = relationship(back_populates="prompt_version")
    tests: Mapped[List["PromptTest"]] = relationship(back_populates="prompt_version", cascade="all, delete-orphan")
    approvals: Mapped[List["AIApproval"]] = relationship(back_populates="prompt_version", cascade="all, delete-orphan")
...
class AIExecution(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    model_version_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("model_versions.id", ondelete="SET NULL"), nullable=True)
    prompt_version_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("prompt_versions.id", ondelete="SET NULL"), nullable=True)
    customer_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("customers.id", ondelete="SET NULL"), nullable=True)
    case_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("cases.id", ondelete="SET NULL"), nullable=True)
    investigation_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("investigations.id", ondelete="SET NULL"), nullable=True)
    sar_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("sars.id", ondelete="SET NULL"), nullable=True)
    risk_score_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("risk_scores.id", ondelete="SET NULL"), nullable=True)
    monitoring_job_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("monitoring_jobs.id", ondelete="SET NULL"), nullable=True)
    report_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("reports.id", ondelete="SET NULL"), nullable=True)
    audit_log_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("audit_logs.id", ondelete="SET NULL"), nullable=True)
    ...
    model_version: Mapped[Optional["ModelVersion"]] = relationship(back_populates="executions")
    prompt_version: Mapped[Optional["PromptVersion"]] = relationship(back_populates="executions")
    feedbacks: Mapped[List["AIFeedback"]] = relationship(back_populates="execution", cascade="all, delete-orphan")
    explanation: Mapped[Optional["AIExplanation"]] = relationship(back_populates="execution", uselist=False, cascade="all, delete-orphan")
...
class AIFeedback(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    execution_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ai_executions.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class ModelEvaluation(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    model_version_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("model_versions.id", ondelete="CASCADE"), nullable=False)
...
class PromptTest(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    prompt_version_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("prompt_versions.id", ondelete="CASCADE"), nullable=False)
...
class AIApproval(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    prompt_version_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("prompt_versions.id", ondelete="CASCADE"), nullable=False)
    reviewer_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
